...
R_Type_Funct = {'add':'100000','addu':'100001','sub':'100010',
	'subu':'100011','and':'100100','or':'100101','xor':'100110',
	'nor':'100111','slt':'10101','sll':'000000','srl':'000010','sra':'000011'}

I_Type_OpCode = {'lui':'001111','addi':'001000','addiu':'001001',
	'andi':'001100','slti':'001010','sltiu':'001011','beq':'000100',
	'bne':'000101','blez':'000110','bgtz':'000111','bgez':'000001',
...
def Binary2Hex(x):						#32 bit Binary str to 8 bit Hex str
	return (format(int(x[:4],2),'x') + format(int(x[4:8],2),'x') + format(int(x[8:12],2),'x') + 
	format(int(x[12:16],2),'x') + format(int(x[16:20],2),'x') + format(int(x[20:24],2),'x') + 
	format(int(x[24:28],2),'x') + format(int(x[28:32],2),'x'))			
...
for line in Data:
	...
	if name in R_Type_Funct:			#deal with R_Type
		...
		Code.append(Binary2Hex(Op + Register[rs] + Register[rt] + Register[rd] + 
			Dec2Binary(shamt,5) + R_Type_Funct[name]))
		...
		Code.append(Binary2Hex(I_Type_OpCode[name] + Register[rs] + 
			Register[rt] + Dec2Binary(imm,16))) 
		...
		if name == 'j' or name == 'jal':
			...
			rs = line[flag + 1]
			Code.append(Binary2Hex('000000' + Register[rs] + '000000000000000' + 
				J_Type_OpCode[name]))
		else:
			rs,rd = line[flag + 1].strip(','),line[flag + 2]
			Code.append(Binary2Hex('000000' + Register[rs] + '00000' + 
				Register[rd] + '00000' + J_Type_OpCode[name]))
